...
const (
	// Redis key prefixes
	RedisKeyPrefix = "bluenet_3ps_backend:"
...
const (
	TokenBlacklistKey = RedisKeyPrefix + "token_blacklist:jti:"
)
...
const (
	...
	FormulaOperationTimeout = 30 * time.Second
)
...
// Payroll Worker Pool Configuration
const (
	MaxPayrollWorkers = 20 // Giới hạn số goroutines đồng thời để xử lý payroll (tăng từ 10 lên 20 để tối ưu performance)
)
...
const (
	...
	ContextActivityLog = "activity_log_ctx" // holds *ActivityLogContext set by service layer
)

const (
	LoginKey = RedisKeyPrefix + "login_attempts:%s"
)
