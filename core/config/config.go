...
func Init(env Environment) error {
	...
	once.Do(func() {
		...
		// MinIO configuration (Mapped to APP_AWS_* for S3/R2 compatibility)
		v.BindEnv("minio.endpoint", "APP_AWS_ENDPOINT")
		v.BindEnv("minio.public_endpoint", "APP_AWS_PUBLIC_ENDPOINT")
		v.BindEnv("minio.access_key_id", "APP_AWS_ACCESS_KEY_ID")
		v.BindEnv("minio.secret_access_key", "APP_AWS_SECRET_ACCESS_KEY")
		v.BindEnv("minio.bucket_name", "APP_AWS_BUCKET")
		v.BindEnv("minio.region", "APP_AWS_REGION")
		v.BindEnv("minio.use_ssl", "APP_MINIO_USE_SSL")
		v.BindEnv("minio.enabled", "APP_MINIO_ENABLED")
