...
func (controller *AuthController) Logout(c echo.Context) error {
	...
	if err != nil {
		return controller.Unauthorized(errors.ErrUnauthorized, "Invalid token", nil)
	}
...
func (controller *AuthController) SendOTPChangePassword(c echo.Context) error {
	...
	if errSend != nil {
		if errSend.Code == errors.ErrUnauthorized {
			return controller.Unauthorized(errSend.Code, errSend.Message, errSend)
		}
		return controller.InternalServerError(errors.ErrInternalServer, "Get OTP change password failed", errSend)
	}
...
func (controller *AuthController) ChangePassword(c echo.Context) error {
	...
	if errUpdate != nil {
		if errUpdate.Code == errors.ErrUnauthorized {
			return controller.Unauthorized(errUpdate.Code, errUpdate.Message, errUpdate)
		}
...
func (controller *AuthController) RefreshToken(c echo.Context) error {
	...
	if errRefresh != nil {
		logger.Error("AuthController:RefreshToken:Error:", errRefresh)
		if errRefresh.Code == errors.ErrUnauthorized {
			return controller.Unauthorized(errRefresh.Code, errRefresh.Message, errRefresh)
		}
		return controller.InternalServerError(errors.ErrInternalServer, "Refresh token failed", errRefresh)
	}
