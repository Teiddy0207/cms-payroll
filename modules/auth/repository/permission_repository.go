...
func (r *AuthRepository) GetUserIDsByPermissionAndDepartment(
	ctx context.Context,
	permission string,
	...
	// CASE 1: GLOBAL permission (không filter department)
	if departmentID == uuid.Nil {
		...
		// CASE 2: DEPARTMENT scope
		query = `
			SELECT DISTINCT u.id
			FROM users u
			JOIN user_profiles up ON up.user_id = u.id
			JOIN job_descriptions jd ON jd.id = up.position_id

			JOIN user_roles ur ON ur.user_id = u.id
			JOIN role_permissions rp ON rp.role_id = ur.role_id
			JOIN permissions p ON p.id = rp.permission_id

			WHERE jd.department_id = $2
			  AND u.is_active = true
			  AND p.slug = $1
		`
		args = []any{permission, departmentID}
	}
