...
// ==========================================
// Job Position Competencies Repository
// ==========================================

func (r *PayrollRepository) AssignCompetencyToPosition(ctx context.Context, jpc *entity.JobPositionCompetency) error {
	query := `
		INSERT INTO job_position_competencies (job_description_id, competency_id, required_level, weight, created_at)
		VALUES (:job_description_id, :competency_id, :required_level, :weight, NOW())
		ON CONFLICT (job_description_id, competency_id) 
		DO UPDATE SET required_level = EXCLUDED.required_level, weight = EXCLUDED.weight
	`
	_, err := r.DB.NamedExecContext(ctx, query, jpc)
	if err != nil {
		logger.Error("PayrollRepository:AssignCompetencyToPosition:Error %v", err)
		return err
	}
	return nil
}

func (r *PayrollRepository) GetCompetenciesByPosition(ctx context.Context, positionID uuid.UUID) ([]entity.JobPositionCompetencyDetail, error) {
	query := `
		SELECT jpc.job_description_id, jpc.competency_id, cd.name AS competency_name, cd.code AS competency_code, jpc.required_level, jpc.weight, cd.point_value
		FROM job_position_competencies jpc
		JOIN competency_dictionaries cd ON jpc.competency_id = cd.id
		WHERE jpc.job_description_id = $1
		ORDER BY cd.code ASC
	`
	var list []entity.JobPositionCompetencyDetail
	err := r.DB.SelectContext(ctx, &list, query, positionID)
	if err != nil {
		if err == sql.ErrNoRows {
			return []entity.JobPositionCompetencyDetail{}, nil
		}
		logger.Error("PayrollRepository:GetCompetenciesByPosition:Error %v", err)
		return nil, err
	}
	return list, nil
}

func (r *PayrollRepository) RemoveCompetencyFromPosition(ctx context.Context, positionID uuid.UUID, competencyID uuid.UUID) error {
	query := `DELETE FROM job_position_competencies WHERE job_description_id = $1 AND competency_id = $2`
	_, err := r.DB.SQLx().ExecContext(ctx, query, positionID, competencyID)
	if err != nil {
		logger.Error("PayrollRepository:RemoveCompetencyFromPosition:Error %v", err)
		return err
	}
	return nil
}
